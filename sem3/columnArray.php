...
$query = $conn->query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'result' AND TABLE_NAME = 'sem_iii'");
...
$result2 = $conn->query("SELECT * FROM `sem_iii` WHERE DSA < 40 OR DBMS < 40 OR PCE < 40 OR LD < 40 OR Maths < 40");
$result3 = $conn->query("SELECT * FROM `sem_iii`");
...
$queryStudent = 'SELECT Student_Name FROM sem_iii ORDER BY Percentage DESC LIMIT 5';
...
$queryPercent = 'SELECT `Percentage` FROM sem_iii ORDER BY Percentage DESC LIMIT 5';
